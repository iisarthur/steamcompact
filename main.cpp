        ...
        for (const auto& entry : fs::directory_iterator(steamappsPath)) {
            if (entry.is_regular_file() && entry.path().extension() == L".acf") {
                ...
                if (game.appid != 0 && !game.name.empty() && game.appid != 228980) {
                    ...
                    if (!game.workingDir.empty() && fs::exists(game.workingDir)) {
                        try {
                            for (const auto& f_entry : fs::recursive_directory_iterator(game.workingDir)) {
                                if (f_entry.is_regular_file() && f_entry.path().extension() == L".exe") {
                                    std::wstring fname = f_entry.path().filename().wstring();
                                    std::transform(fname.begin(), fname.end(), fname.begin(), ::tolower);
                                    game.exeNames.push_back(fname);
                                }
                            }
                        } catch (...) {}
                    }
...
void SetProcessPriorityByName(const std::wstring& processName, DWORD priorityClass) {
    HANDLE hSnapshot = CreateToolhelp32Snapshot(TH32CS_SNAPPROCESS, 0);
    if (hSnapshot != INVALID_HANDLE_VALUE) {
        PROCESSENTRY32W pe;
        pe.dwSize = sizeof(PROCESSENTRY32W);
    ...
    switch (message) {
        ...
        case WM_TIMER: {
            if (wParam == 1) {
                    ...
                    if (game.isRunning) {
                        ...
                        if (isPriorityEnabled) {
                            for (const auto& exe : game.exeNames) {
                                SetProcessPriorityByName(exe, HIGH_PRIORITY_CLASS);
                            }
                        }
                    }
                }
                ...
                for (auto& game : displayedGames) {
                    game.isRunning = false;
                    for (const auto& exe : game.exeNames) {
                        if (runningProcesses.count(exe) > 0) {
                            game.isRunning = true;
                            break;
                        }
                    }
        ...
        case WM_DRAWITEM: {
            ...
            if (pDIS->CtlID == IDC_PLAY_BUTTON) {
                ...
                COLORREF bgNormal = isStop ? RGB(140, 20, 20) : RGB(45, 45, 45); 
                ...
                HPEN hPen = CreatePen(PS_SOLID, 1, isStop ? RGB(200, 50, 50) : RGB(70, 70, 70));
        ...
        case WM_COMMAND: {
            ...
            if (wmId == IDC_SEARCH_EDIT && wmEvent == EN_CHANGE) {
                ...
                if (idx != -1 && idx < (int)displayedGames.size()) {
                    if (displayedGames[idx].isRunning) {
                        for (const auto& exeName : displayedGames[idx].exeNames) {
