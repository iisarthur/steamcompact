...
void LoadToggleStates(bool& overlayEnabled, bool& closeSteamEnabled, bool& killerEnabled) {
  overlayEnabled = true;
  ...
  if (RegOpenKeyEx(HKEY_CURRENT_USER, L"Software\\SteamCompact", 0, KEY_READ, &hKey) == ERROR_SUCCESS) {
    DWORD overlayVal = 1, closeVal = 0, killerVal = 0;
...
void SaveToggleStates(bool overlayEnabled, bool closeSteamEnabled, bool killerEnabled) {
  HKEY hKey;
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam) {
  switch (message) {
    case WM_CREATE: {
      ...
      bool overlayDefault = true, closeSteamDefault = false, killerDefault = false;
      LoadToggleStates(overlayDefault, closeSteamDefault, killerDefault);
    ...
    case WM_CTLCOLORSTATIC: {
      ...
      } else if (hwndStatic == hCheckboxOverlay || hwndStatic == hCheckboxCloseSteam || hwndStatic == hCheckboxKiller) {
    ...
    case WM_COMMAND: {
      ...
      if (wmId == IDC_SEARCH_EDIT && wmEvent == EN_CHANGE) {
      ...
      else if (wmId == IDC_OVERLAY_CHECKBOX || wmId == IDC_CLOSE_STEAM_CHECKBOX || wmId == IDC_KILLER_CHECKBOX) {
        ...
        SaveToggleStates(overlay, closeSteam, killer);
    ...
    case WM_NOTIFY: {
      ...
      break;
            ...
            case WM_SIZE: {
              ...
              MoveWindow(hListView, 10, 65, width - 20, height - 207, TRUE);
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nCmdShow) {
  ...
  HWND hWnd = CreateWindowW(L"SteamCompactLauncherClass", GetStrings().windowTitle.c_str(),
                            WS_OVERLAPPEDWINDOW, CW_USEDEFAULT, CW_USEDEFAULT, 340, 560, NULL, NULL, hInstance, NULL);
